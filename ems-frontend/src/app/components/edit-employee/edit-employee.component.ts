...
export class EditEmployeeComponent implements OnInit {
  ...
  resetPassword() {
    ...
    const empId = this.route.snapshot.paramMap.get('id') || this.employee.id;

    this.http.put(
      `https://employee-management-system-3-ywre.onrender.com/api/employees/reset-password/${empId}`,
      {},
      { headers, responseType: 'text' }
    )
    .subscribe({
